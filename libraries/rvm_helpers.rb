...
#
 module RVMHelpers
    ...
    def find_profile_to_source_my(user_dir = nil)
      if user_dir
    ...
    def rvm_wrap_cmd_my(cmd, user_dir = nil)
      profile = find_profile_to_source_my(user_dir)
      %{bash -c "source #{profile} && #{cmd.gsub(/"/, '\"')}"}
    end

    def ruby_path
        cmd = 'which ruby'
        shell_out!(rvm_wrap_cmd_my(cmd)).stdout.strip
    end
    ...
    def ruby_wrapper
        gemdir = gem_dir
        wrapper_home = gemdir.sub(/\/gems\//, '/wrappers/')
        return "#{wrapper_home}/ruby"
    end
 end
